...
class Graph:
    ...
    def build_copy(self):
        ...
        while queue:
            node = queue.pop(0)
            cpy_node = Node()
            cpy_node.label = node.label
            cpy_node.paths = node.paths
            cpy_node.children = node.children
            cpy_node.source_edge = node.source_edge
            cpy_node.source_node = node.source_node
            cpy_node.identifier = node.identifier
            cpy_node.string = node.string
            graph.append(node)
            for sub_node in node.children:
                # print(i)
                if sub_node in self.final_graph:
...
def BuildPrefixTree(strings, labels, I):
    ...
    while queue:
        ...
        if node.label != None:
            pass
            #print(node.identifier, '--', node.label, '--', node.string, i)
        for sub_node in node.children:
            queue.append(sub_node)
    prefix_tree.breadth_first_search_tree()
    graph = Trachtenbrot_Barzdin_algorithm(prefix_tree, I)
    return graph

def alphabet():
    return [0, 1]

def breadth_first_search_graph(graph, unique_nodes, I):
...
def adjacency_matrix(unique_nodes):
    pass
def Trachtenbrot_Barzdin_algorithm(prefix_tree, I):
    prefix_tree.breadth_first_search_tree()

    graph = Graph(prefix_tree)
    unique_nodes = []
    unique_nodes.append(graph.root)
    breadth_first_search_graph(graph, unique_nodes, I)
    graph.build_adjacency_matrix()
...
def MatchLabels(potential_node, unique_node):
    ...
    if (potential_node.label != unique_node.label) and (potential_node.label is not None or unique_node.label is not None):
        #print(f'False {potential_node.label} -- {unique_node.label}')
        return False

    if (unique_node.label is None) and (potential_node.label is not None):
        print(unique_node.label, '---', potential_node.label)
        unique_node.label = potential_node.label
...
def BacktrackingApproach(strings, labels, I):
    graph = BuildPrefixTree(strings, labels, I)
    node = 0
    DFAComplete(I, graph, node)

def CreateDFA(I, A, row, col):
    pass

def CheckCurrent(I, row, col, k):
    pass


def DFAComplete(I, graph, node):
    for num in range(len(I) - 1):
        for j in range(len(graph.adjacency_matrix)):
            if I[node][num] - 1 != graph[node].index(num):
                return False
            else:
                node = I[node][num]
                DFAComplete(I, graph, node)
    ...
    return True
def CopyLabels(i, j):
    # copy the state labels from the subtree
    # rooted at i into the subtree rooted at j.
    pass
